...
import { Trophy, Shield, CalendarDays, Sparkles, ChevronDown } from "lucide-react";
...
function CareerTimeline() {
  ...
  const seasonsByClub = useMemo(() => {
    ...
    for (const [clubId, list] of map.entries()) {
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="space-y-6">
        {sortedClubs.map((club) => {
          ...
          const clubTrophies = trophies.filter((t) => t.clubId === club.id);
          ...
          >
              <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                <div className="flex items-center gap-2">
                  <div className="h-12 w-12 rounded-3xl border border-white/10 shadow-sm overflow-hidden bg-white/5">
              ...
              <div className="mt-4 space-y-2">
                {clubSeasons.length === 0 ? (
                  ...
                  clubSeasons.map((season) => {
                    const trophiesForSeason = clubTrophies.filter((t) => t.year === season.year);
                    return (
                      <div
                        key={season.id}
                      ...
                      >
                        <Collapsible>
                          <CollapsibleTrigger asChild>
                            <button className="w-full p-2 flex flex-col gap-1 md:flex-row md:items-center md:justify-between hover:opacity-80 transition-opacity text-left">
                              <div className="flex items-center gap-2">
                                  ...
                                  >
                                    <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                                      ...
                                      {competition.won ? (
                                        ...
                                        <span className="inline-flex items-center gap-1 rounded-full bg-accent text-accent-foreground px-2 py-0.5 text-[10px] font-semibold">
                                          Finalista
                                        </span>
                                      ) : null}
                                    </div>
                                ...
                                {trophiesForSeason.map((trophy) => (
                                  <div key={trophy.id} className="rounded-3xl border border-primary/60 bg-primary/10 p-2">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <div>
                                        <div className="font-semibold text-xs">{trophy.competition}</div>
